...
package body Exp_Ch9 is
   ...
   function Build_Barrier_Function_Specification
     (Loc      : Source_Ptr;
      Def_Id   : Entity_Id;
      Obj_Type : Entity_Id) return Node_Id;
   ...
   --  entry body. Essentially the same code as Build_Protected_Subprogram_Body
   --  but modified to call entry specific code. This code could really be
   --  included in Build_Protected_Subprogram_Body.

   function Build_Protected_Entry_Specification
     (N        : Node_Id;
      Prot_Typ : Entity_Id;
      Mode     : Subprogram_Protection_Mode) return Node_Id;
   --  Build a specification for the protected entry. Handles both the
   --  protected and unprotected subprograms.
         ...
         begin
            ...
            Decl :=
              Make_Object_Declaration (Loc,
                ...
                Object_Definition   =>
                  New_Occurrence_Of (RTE (RE_Task_List), Loc),
                Expression          =>
                  New_Occurrence_Of (RTE (RE_No_Agent), Loc));
                     ...
                     Make_Simple_Return_Statement (Loc,
                       Expression =>
                         Make_Function_Call (Loc,
                           Name                   =>
                             New_Occurrence_Of (Barrier_Function (Ent),
                                                Loc),
   ...
   begin
      P_Op_Spec :=
        Build_Protected_Entry_Specification (N, Pid, Protected_Mode);
      ...
      Stmts := New_List (
        Make_Procedure_Call_Statement (Loc,
          Name                   =>
            New_Occurrence_Of (RTE (RE_Enter_Protected_Object), Loc),
          Parameter_Associations =>  New_List (Object_Parm,
            New_Occurrence_Of (RTE (RE_Protected_Entry), Loc),
      ...
      return Sub_Body;
   end Build_Protected_Entry_Body;
   ...
   begin
      if Nkind (N) = N_Entry_Body then
         Decl := Unit_Declaration_Node
           (Corresponding_Spec (Entry_Body_Formal_Part (N)));
      else
         Decl := N;
      end if;

      --  An entry has the same spec format as a protected procedure with the
      --  addition of the entry index. Thus we reuse the Build_Protected_Spec
      --  function as a starting point for the spec.
      New_Plist :=
      ...
      New_Pram :=
         Make_Parameter_Specification (Loc,
           Defining_Identifier =>
             Make_Defining_Identifier (Loc, Name_uEntry_Id),
           Parameter_Type =>
             New_Occurrence_Of (RTE (RE_Protected_Entry_Index), Loc));
   ...
   begin
      ...
      if Nkind (Op_Spec) = N_Function_Specification then
         Sub_Type := New_Occurrence_Of (RTE (RE_Protected_Function), Loc);
      else
         Sub_Type := New_Occurrence_Of (RTE (RE_Protected_Procedure), Loc);
      end if;
      ...
      if not Exc_Safe then
         ...
         Append_To (Stmts,
           Make_Procedure_Call_Statement (Loc,
             Name                   =>
               New_Occurrence_Of (RTE (RE_Exit_Protected_Object), Loc),
             Parameter_Associations =>
   ...
   begin
      ...
      begin
         ...
         if not Is_Protected_Type (Etype (Concval)) then
            raise Program_Error
              with "Calls to task entries are not supported.";
         end if;

         New_Sub :=
           New_Occurrence_Of (External_Subprogram (Ent), Loc);
   ...
   begin
      ...
      return
        Make_Subprogram_Body (Sloc (N),
          Specification              =>
            Build_Protected_Entry_Specification (N, Pid, Unprotected_Mode),
          Declarations               => Decls,
          Handled_Statement_Sequence => Handled_Statement_Sequence (N));
   end Build_Unprotected_Entry_Body;
   ...
   --  function to calculate each entry barrier. For example, for
   ...
   --  procedure entN (_object : in out poV; Entry_Id : in Entry_Index ...) is
   --     <discriminant renamings>
   --     <private object renamings>
   --  begin
   --     <sequence of statements>
   ...
   --  procedure entP (_object : in out poV; Entry_Id : in Entry_Index ...) is
   --     procedure _finalizer is
   --     begin
   --       poREB (_object);
   --       Exit_Protected_Object (_object._protected_agent);
   ...
   --  procedure poR (_object : in poV) is
   --  begin
   --     _object._barrier (1) := entB (_object : in poV, 1);
   --  end poR;
   ...
   begin
      ...
      while Present (Op_Body) loop
         case Nkind (Op_Body) is
            ...
            when N_Entry_Body =>
               New_Op_Body :=
                 Build_Unprotected_Entry_Body (Op_Body, Pid);
   ...
   procedure Expand_N_Protected_Type_Declaration (N : Node_Id) is
      ...
      --  Move preconditions and postconditions from the entry definition to
      --  the protected subprogram. This is carried out as the preconditions
   ...
   begin
      ...
      --  If we have entries, make the the declaration for the barrier service
      --  subprogram.
         ...
         Size_Decl :=
           Make_Object_Declaration (Loc,
             ...
             Object_Definition =>
               New_Occurrence_Of (RTE (RE_Storage_Count), Loc),
             Expression =>
               New_Occurrence_Of (RTE (RE_Unspecified_Call_Stack_Size), Loc));
      end if;
        ...
        Make_Component_Declaration (Loc,
          ...
          Component_Definition =>
            Make_Component_Definition (Loc,
              Aliased_Present    => False,
              Subtype_Indication =>
                New_Occurrence_Of (RTE (RE_Task_Id), Loc))));
           ...
           Make_Component_Declaration (Loc,
             ...
             Component_Definition =>
               Make_Component_Definition (Loc,
                 Aliased_Present    => False,
                 Subtype_Indication =>
                   New_Occurrence_Of (RTE (RE_Storage_Count),
                                     Loc)),
   ...
   begin
      ...
      --  formal parameter _object or _task depending on the context.
   ...
   begin
      ...
      if not Uses_Lock_Free (Defining_Identifier (Pdec)) then
         ...
         if Present (Barrier_Service_Function (
                       Defining_Identifier (Pdec)))
         then
            Append_To (Args,
              Make_Attribute_Reference (Loc,
                Prefix =>
                  New_Occurrence_Of (Barrier_Service_Function
      ...
      if Has_Attach_Handler (Ptyp) then
         ...
         begin
            ...
            Append_To (L,
              Make_Procedure_Call_Statement (Loc,
               Name => New_Occurrence_Of (RTE (RE_Attach_Handlers), Loc),
               Parameter_Associations => Args));
         end;
      end if;
   ...
   begin
      ...
      Name := New_Occurrence_Of (RTE (RE_New_Task_Agent), Loc);
   ...
   begin
      ...
      Append_To (Decls,
        Make_Object_Declaration (Loc,
          ...
          Constant_Present    => True,
          Object_Definition   => New_Occurrence_Of (RTE (RE_Behaviour), Loc),
          Expression          => Expr));
      ...
      if Has_Rep_Item (Ttyp, Name_Cycle_Period, Check_Parents => False) then
         ...
         Expr := New_Occurrence_Of (RTE (RE_Time_Span_Last), Loc);
      end if;
      ...
      Append_To (Decls,
        Make_Object_Declaration (Loc,
          ...
          Constant_Present    => True,
          Object_Definition   => New_Occurrence_Of (RTE (RE_Time_Span), Loc),
          Expression          => Expr));
      ...
      if Has_Rep_Item (Ttyp, Name_Cycle_Phase, Check_Parents => False) then
         ...
         Expr := New_Occurrence_Of (RTE (RE_Time_Span_Zero), Loc);
      end if;
      ...
      if Has_Rep_Item (Ttyp, Name_Budget_Response, Check_Parents => False) then
         ...
         Expr := New_Occurrence_Of (RTE (RE_No_Response), Loc);
      end if;
      ...
      Append_To (Decls,
        Make_Object_Declaration (Loc,
          ...
          Constant_Present    => True,
          Object_Definition   => New_Occurrence_Of (RTE (RE_Event_Response),
                                                   Loc),
          Expression          => Expr));
      ...
      then
         ...
         Expr := New_Occurrence_Of (RTE (RE_No_Agent), Loc);
      end if;
   ...
   begin
      ...
      Append_To (Args, New_Occurrence_Of (Exec_Object, Loc));
      ...
      then
         ...
         Expr := New_Occurrence_Of (RTE (RE_Default_Priority), Loc);
      end if;
